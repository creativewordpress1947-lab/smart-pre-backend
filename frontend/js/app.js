...
(function () {
   ...
   async function fetchInitialData() {
      const apiBase = window.SmartPreConfig ? window.SmartPreConfig.apiBaseUrl : '';
      try {
         const priceResp = await fetch(`${apiBase}/api/prices`);
         if (priceResp.ok) {
      ...
      try {
         const decResp = await fetch(`${apiBase}/api/decisions`);
         if (decResp.ok) {
   ...
   async function loadChartData(symbol) {
      ...
      try {
         ...
         const histResp = await fetch(`${apiBase}/api/history/${symbol}?timeframe=${tf}`);
         if (histResp.ok) {
